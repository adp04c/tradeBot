...
from rippleTrade import rippleLib , folderLocation
...
decsion = []
for p in ["buy","sell"]:   
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "model4rf.pk1") 
    rf4 = c.predict_proba(records[0])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "model4clf.pk1") 
    clf4 = c.predict_proba(records[0])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model8rf.pk1") 
    rf8 = c.forest.predict_proba(records[1])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model8clf.pk1") 
    clf8 = c.clf.predict_proba(records[1])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model12rf.pk1") 
    rf12 =c.forest.predict_proba(records[2])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model12clf.pk1") 
    clf12 = c.clf.predict_proba(records[2])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model16rf.pk1") 
    rf16 = c.forest.predict_proba(records[3])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model16clf.pk1") 
    clf16 = c.clf.predict_proba(records[3])
    c = joblib.load(folderLocation +"models/" + p + "/" + p + "models/model4clf.pk1") 
    d = c.predict_proba(records[0])
    decsion.append(d)
